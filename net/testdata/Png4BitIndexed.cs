...
namespace PeterO
{
	...
	public sealed class Png4BitIndexed
	{
		...
		public void SetPixel(int x,int y,int pixel){
			...
			int offset=(y*this.realRowSize)+(x>>1)+1;
			if((x&1)==1){
		...
		public int GetPixel(int x,int y){
			...
			int offset=(y*this.realRowSize)+(x>>1)+1;
			if((x&1)==0)
				return this.data[offset]&0x0F;
			else
				return (this.data[offset]>>4)&0x0F;
		}

		public byte[] GetColor(int index){
			if(index<0||index>=16)throw new ArgumentOutOfRangeException("index");
			return new byte[]{
				colors[index*3],
				colors[index*3+1],
